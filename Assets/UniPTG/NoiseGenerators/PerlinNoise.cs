using System.Collections;
using System.Collections.Generic;
using UnityEngine;

namespace UniPTG.NoiseGenerators
{
    internal class PerlinNoise : NoiseGeneratorBase
    {
        private const float NoiseFrequency = 256f;

        //シード値
        [SerializeField]
        private float _z;

        //疑似乱数順列P
        static readonly int[] p = new int[512];
        static readonly int[] permutation = { 151,160,137,91,90,15,
           131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,21,10,23,
           190, 6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,
           88,237,149,56,87,174,20,125,136,171,168, 68,175,74,165,71,134,139,48,27,166,
           77,146,158,231,83,111,229,122,60,211,133,230,220,105,92,41,55,46,245,40,244,
           102,143,54, 65,25,63,161, 1,216,80,73,209,76,132,187,208, 89,18,169,200,196,
           135,130,116,188,159,86,164,100,109,198,173,186, 3,64,52,217,226,250,124,123,
           5,202,38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,182,189,28,42,
           223,183,170,213,119,248,152, 2,44,154,163, 70,221,153,101,155,167, 43,172,9,
           129,22,39,253, 19,98,108,110,79,113,224,232,178,185, 112,104,218,246,97,228,
           251,34,242,193,238,210,144,12,191,179,162,241, 81,51,145,235,249,14,239,107,
           49,192,214, 31,181,199,106,157,184, 84,204,176,115,121,50,45,127, 4,150,254,
           138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,61,156,180
        };

        //Pの初期化
        static PerlinNoise()
        {
            for (int i = 0; i < 256; i++)
            {
                p[256 + i] = permutation[i];
                p[i] = permutation[i];
            }
        }

        public override float GetValue(float x, float y)
        {
            float z = _z;

            //格子点の座標を求める
            int xi = (int)x & 255;
            int yi = (int)y & 255;
            int zi = (int)z & 255;

            //格子点内の座標を求める
            x -= (int)x;
            y -= (int)y;
            z -= (int)z;

            //格子点からハッシュ値を算出
            int aaa = p[p[p[xi] + yi] + zi];
            int baa = p[p[p[xi + 1] + yi] + zi];
            int aba = p[p[p[xi] + yi + 1] + zi];
            int bba = p[p[p[xi + 1] + yi + 1] + zi];
            int aab = p[p[p[xi] + yi] + zi + 1];
            int bab = p[p[p[xi + 1] + yi] + zi + 1];
            int abb = p[p[p[xi] + yi + 1] + zi + 1];
            int bbb = p[p[p[xi + 1] + yi + 1] + zi + 1];

            //勾配ベクトルと距離ベクトルの内積を求める
            float gaaa = Grad(aaa, x, y, z);
            float gbaa = Grad(baa, x - 1, y, z);
            float gaba = Grad(aba, x, y - 1, z);
            float gbba = Grad(bba, x - 1, y - 1, z);
            float gaab = Grad(aab, x, y, z - 1);
            float gbab = Grad(bab, x - 1, y, z - 1);
            float gabb = Grad(abb, x, y - 1, z - 1);
            float gbbb = Grad(bbb, x - 1, y - 1, z - 1);

            //加重平均の係数を算出
            float u = Fade(x);
            float v = Fade(y);
            float w = Fade(z);

            //加重平均を算出
            return Lerp(w,
                    Lerp(v,
                        Lerp(u, gaaa, gbaa),
                        Lerp(u, gaba, gbba)),
                    Lerp(v,
                        Lerp(u, gaab, gbab),
                        Lerp(u, gabb, gbbb)));
        }

        //補完関数
        private float Fade(float t)
        {
            return t * t * t * (t * (t * 6 - 15) + 10);
        }

        //勾配ベクトルと距離ベクトルの内積を求める
        private float Grad(int hash, float x, float y, float z)
        {
            switch (hash & 0xF)
            {
                case 0x0: return x + y;
                case 0x1: return -x + y;
                case 0x2: return x - y;
                case 0x3: return -x - y;
                case 0x4: return x + z;
                case 0x5: return -x + z;
                case 0x6: return x - z;
                case 0x7: return -x - z;
                case 0x8: return y + z;
                case 0x9: return -y + z;
                case 0xA: return y - z;
                case 0xB: return -y - z;
                case 0xC: return y + x;
                case 0xD: return -y + z;
                case 0xE: return y - x;
                case 0xF: return -y - z;
                default: return 0; // never happens
            }
        }

        //加重平均
        private float Lerp(float t, float a, float b)
        {
            return a + t * (b - a);
        }

        public override void InitState()
        {
        }

        public override void UpdateState()
        {
        }
    }
}
